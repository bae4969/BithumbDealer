...
namespace BithumbDealer.form
{
    public partial class History : Form
    {
        ...
        private void History_Load(object sender, EventArgs e)
        {
            ...
            macroPrint.Columns.Add("unit", typeof(float));
            macroPrint.Columns.Add("price", typeof(float));
            macroPrint.Columns.Add("target", typeof(float));

            macroPrintSwap.Columns.Add("coinName", typeof(string));
            macroPrintSwap.Columns.Add("date", typeof(DateTime));
            macroPrintSwap.Columns.Add("unit", typeof(float));
            macroPrintSwap.Columns.Add("price", typeof(float));
            macroPrintSwap.Columns.Add("target", typeof(float));
                    ...
                    switch (type)
                    {
                        ...
                        case 1:
                            if (isNeedUpdateHistory)
                            {
                                ...
                                int ret = ((Main)Owner).tradeHistory.updateHistoryData(text_historyCoinName.Text, offset);
                                if (ret < 0)
                                {
                        ...
                        case 2:
                            ...
                            for (int i = 0; i < macroState.Tables.Count; i++)
                            {
                                ...
                                for (int j = 0; j < macroState.Tables[tempCoinName].Rows.Count; j++)
                                {
                                    ...
                                    dataRow["target"] = (float)macroState.Tables[tempCoinName].Rows[j]["price"]
                                        * (100f + macroSetting.yield) / 100f;
                ...
                switch (type)
                {
                    ...
                    case 1:
                        if (isNeedBindHistory)
                        {
                            ...
                            text_offset.ForeColor = Color.Gray;
        ...
        private void dataGridView1_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (type == 0 && dataGridView1.GetCellCount(DataGridViewElementStates.Selected) > 0)
                lock (((Main)Owner).lock_tradeHistory)
                {
                    ...
                    selected.id = (ulong)pendingData.Rows[row]["id"];
                    ...
                    selected.unit = (float)pendingData.Rows[row]["unit"];
        ...
        private void btn_cancel_Click(object sender, EventArgs e)
        {
            if (type == 0 && selected.id > 0)
            {
                ...
                {
                    int ret = ((Main)Owner).tradeHistory.cancelPending(selected.isBid, selected.id, selected.coinName);
                    if (ret < 0)
                    {
        ...
        {
            if (!int.TryParse(text_offset.Text, out offset))
                MessageBox.Show("Offset must be intger type value.");
            else
            {
